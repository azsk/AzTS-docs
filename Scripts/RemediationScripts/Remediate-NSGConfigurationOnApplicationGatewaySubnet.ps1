...
function Add-NSGConfigurationOnSubnet
{
    ...
    $ApplicationGatewaySubnetDetails = @()
    ...
    if ([String]::IsNullOrWhiteSpace($FilePath))	
        ...
        $ApplicationGatewaySubnetDetails =  Get-AzApplicationGateway -ErrorAction Stop
        ...
        Write-Host $([Constants]::SingleDashLine)
        $ApplicationGatewaySubnetResources = Import-Csv -LiteralPath $FilePath

        $validApplicationGatewaySubnetResources = $ApplicationGatewaySubnetResources| Where-Object { ![String]::IsNullOrWhiteSpace($_.ResourceId) }

        $validApplicationGatewaySubnetResources| ForEach-Object {
            $resourceId = $_.ResourceId

            try
            {   
                $ApplicationGatewaySubnetDetails =  Get-AzApplicationGateway -ResourceGroupName $_.ResourceGroupName -Name $_.ResourceName -ErrorAction SilentlyContinue
                $ApplicationGatewaySubnetDetails = $ApplicationGatewaySubnetDetails | Select-Object @{N='ResourceId';E={$_.Id}},
                                                                          @{N='ResourceGroupName';E={$_.Id.Split("/")[4]}},
                                                                          @{N='ResourceName';E={$_.Name}},
                                                                          @{N='ResourceSubNetId';E={
    ...
    if ($totalApplicationGatewaySubnet -eq 0)
    {
        Write-Host "No Application Gateway(s) found. Exiting..." -ForegroundColor $([Constants]::MessageType.Update)
        Write-Host $([Constants]::DoubleDashLine)	
        return
    }
  
    Write-Host "Found [$($totalApplicationGatewaySubnet)] Subnet(s)." -ForegroundColor $([Constants]::MessageType.Update)
    ...
    $ApplicationGatewaySubnetDetails | ForEach-Object {
        $Subnet = $_        
