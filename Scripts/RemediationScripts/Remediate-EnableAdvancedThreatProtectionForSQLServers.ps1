...
function Enable-AdvancedThreatProtectionForSqlServers
{
    ...
    Write-Host "[Step 1 of 4] Prepare to enable Auditing and Advanced Threat Protection for SQL Server(s) in Subscription: [$($SubscriptionId)]"
    Write-Host $([Constants]::SingleDashLine)
    ...
    Write-Host "To enable Auditing and Advanced Threat Protection for SQL Server(s) in a Subscription, Contributor and higher privileges on the SQL Server(s) in the Subscription are required. " -ForegroundColor $([Constants]::MessageType.Warning)
    ...
    if (-not $DryRun)
    {
        ...
        Write-Host "[Step 4 of 4] Enable Auditing and Advanced Threat Protection for SQL Servers"
        Write-Host $([Constants]::SingleDashLine)
...
function Disable-AdvancedThreatProtectionForSqlServers
{
    ...
    Write-Host $([Constants]::DoubleDashLine)
    Write-Host "[Step 1 of 3] Prepare to disable Auditing and Advanced Threat Protection for SQL Servers in Subscription: [$($SubscriptionId)]"
    Write-Host $([Constants]::SingleDashLine)
    if ($PerformPreReqCheck)
    {
        try
    ...
    Write-Host "[Step 3 of 3] Disable Auditing and Advanced Threat Protection for SQL Servers"
    Write-Host $([Constants]::SingleDashLine)
    ...
    $isAtpEnabledAtSubscriptionLevel = $validSqlServerDetails[0].IsAtpEnabledAtSubscriptionLevel
        ...
        try
        {
            ...
            if ($isAuditingEnabled -eq $false)
            {
                ...
                if ($sqlServerInstance.IsSynapseWorkspace -eq $false)
                {
                    ...
                    $sqlServerAuditDetails = Remove-AzSqlServerAudit -ResourceGroupName $sqlServerInstance.ResourceGroupName -ServerName $sqlServerInstance.ServerName -ErrorAction Continue
                }
                else
                {
                    # SQL Server is associated with a Synapse Workspace.
                    # Synapse Workspace and the associated SQL Server have the same name.
                    # Any exceptions when disabling Auditing on the SQL Server will be caught and the SQL Server will be considered as skipped for roll back.
                    $sqlServerAuditDetails = Reset-AzSynapseSqlAuditSetting -ResourceGroupName $sqlServerInstance.ResourceGroupName -WorkspaceName $sqlServerInstance.ServerName -ErrorAction Continue
                }
