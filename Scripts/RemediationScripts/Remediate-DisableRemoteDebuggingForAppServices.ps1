    ...
    $appServiceResources | ForEach-Object {
        ...
        try
        {
            ...
            if ($isRemoteDebuggingDisabledOnProductionSlot -and $isRemoteDebuggingDisabledOnAllNonProductionSlots)
            {
                ...
                Write-Host "Remote Debugging is disabled on the production slot and all non-production slots in the App Service." -ForegroundColor $([Constants]::MessageType.Update)
                return
            }
    ...
    if (-not $DryRun)
    {
        ...
        if($AutoRemediation){
            if ($($appServicesRemediated | Measure-Object).Count -gt 0)
            {
                ...
                Write-Host "The information related to App Service(s) where Remote debugging successfully disabled has been saved to [$($appServicesRemediatedFile)]. Use this file for any roll back that may be required." -ForegroundColor $([Constants]::MessageType.Info)
            }
            ...
            {   
                ...
                Write-Host "The information related to App Service(s) where Remote debugging not disabled has been saved to $($appServicesSkippedFile)"
            }
        }else{
...
function Enable-RemoteDebuggingForAppServices
{
    ...
    $validAppServiceDetails | ForEach-Object {
        ...
        try
        {
            ...
            $nonProductionSlotsSkipped = $nonProductionSlotsWithoutRemoteDebuggingDisabled
            $nonProductionSlotsSkippedStr = $($nonProductionSlotsWithoutRemoteDebuggingDisabled -join ', ')
            $appService.NonProductionSlotsSkipped = $nonProductionSlotsSkippedStr
            ...
            $appService.NonProductionSlotsWithoutRemoteDebuggingDisabled = $nonProductionSlotsSkippedStr
            $appService.NonProductionSlotsSkipped = $nonProductionSlotsSkippedStr
            ...
            if ($($nonProductionSlotsSkipped | Measure-Object).Count -eq 0)
            {
                ...
                $appServicesRolledBack += $appService | Select-Object @{N='ResourceID';E={$_.ResourceId}},
                                                                        ...
                                                                        @{N='NonProductionSlotsWithoutRemoteDebuggingDisabled';E={$_.NonProductionSlotsWithRemoteDebuggingDisabled}},
        ...
        catch
        {
            $appServicesSkipped += $appService | Select-Object @{N='ResourceID';E={$resourceId}},
                                                                ...
                                                                @{N='NonProductionSlotsWithoutRemoteDebuggingDisabled';E={$nonProductionSlotsWithRemoteDebuggingDisabled}},
    ...
    {
        Write-Host "Remote debugging successfully enabled for the following App Service(s):"
        Write-Host $([Constants]::SingleDashLine)
