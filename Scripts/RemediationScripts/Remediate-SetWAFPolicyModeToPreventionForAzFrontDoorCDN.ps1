<###
# Overview:
    This script is used to set the WAF Policy mode to Prevention Configured on Front Door CDN(s) in a Subscription.
...
# Steps performed by the script:
    To remediate:
        ...
        2. Get the list of all Front Door CDNs in a Subscription that do not have WAF Policy mode in Prevention.
        3. Back up details of Front Door CDNs that are to be remediated.
        4. Change the WAF policy mode configured on endpoints domain(s) in the Front Door CDNs.
    ...
    To roll back:
        ...
        3. Revert by changing the WAF Policies mode configured on endpoint domain(s) in all the Front Door CDNs.

# Instructions to execute the script:
    To remediate:
        1. Download the script.
        2. Load the script in a PowerShell session. Refer https://aka.ms/AzTS-docs/RemediationscriptExcSteps to know more about loading the script.
        3. Execute the script to change WAF Policy mode configured on all endpoint domain(s) of Front Door CDNs in a Subscription. Refer `Examples`, below.

    To roll back:
        1. Download the script.
        2. Load the script in a PowerShell session. Refer https://aka.ms/AzTS-docs/RemediationscriptExcSteps to know more about loading the script.
        3. Execute the script to roll back the WAF Policy mode Configurated on all endpoint domain(s) of Front Door CDNs in a Subscription- we previously remediated. Refer `Examples`, below.

# Examples:
    To remediate:
        1. To review the Front Door CDNs in a Subscription that will be remediated:
           SetPreventionMode-OnWAFPolicyOfAzureFrontDoorCDN -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -DryRun

        2. To set WAF Policy mode configured on Endpoint domain(s) of all Front Door CDNs in a Subscription:
           SetPreventionMode-OnWAFPolicyOfAzureFrontDoorCDN -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck
        ...
        3. To set WAF Policy mode configured on Endpoint domain(s) of all Front Door CDNs in a Subscription, from a previously taken snapshot:
           SetPreventionMode-OnWAFPolicyOfAzureFrontDoorCDN -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -FilePath C:\AzTS\Subscriptions\00000000-xxxx-0000-xxxx-000000000000\202109131040\SetPolicyStateToEnabled\FrontDoorCDNSecurityPolicyDetailsBackUp.csv

        To know more about the options supported by the remediation command, execute:
        Get-Help SetPreventionMode-OnWAFPolicyOfAzureFrontDoorCDN -Detailed
    ...
    To roll back:
        1. To revert WAF Policy mode configured on Endpoint domain(s) of all Front Door CDNs in a Subscription, from a previously taken snapshot:
           SetDetectionMode-OnWAFPolicyOfAzureFrontDoorCDN -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -FilePath C:\AzTS\Subscriptions\00000000-xxxx-0000-xxxx-000000000000\202109131040\SetPolicyStateToEnabled\RemediatedFrontDoorCDNPolicy.csv
        
        To know more about the options supported by the roll back command, execute:
        Get-Help SetDetectionMode-OnWAFPolicyOfAzureFrontDoorCDN -Detailed        
###>
...
function SetPreventionMode-OnWAFPolicyOfAzureFrontDoorCDN
{
    <#
        .SYNOPSIS
        ...
        .DESCRIPTION
        Remediates 'Azure_FrontDoor_CDNProfile_NetSec_Enable_WAF_Configuration' Control.
        Set WAF Policy mode to Prevention configured on Endpoint domain(s) in Front Door CDNs(s) in the Subscription. 
        ...
        .INPUTS
        None. You cannot pipe objects to SetPreventionMode-OnWAFPolicyOfAzureFrontDoorCDN.

        .OUTPUTS
        None. SetPreventionMode-OnWAFPolicyOfAzureFrontDoorCDN does not return anything that can be piped and used as an input to another command.

        .EXAMPLE
        PS> SetPreventionMode-OnWAFPolicyOfAzureFrontDoorCDN -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -FilePath C:\AzTS\Subscriptions\00000000-xxxx-0000-xxxx-000000000000\202211190719\SetPolicyStateToEnabled\FrontDoorCDNSecurityPolicyDetailsBackUp.csv
    ...
    Write-Host "To change WAF Policy mode for all Front Door CDNs(s) in a Subscription, Contributor or higher privileges role on the Front Door CDNs are required." -ForegroundColor $([Constants]::MessageType.Info)
    Write-Host $([Constants]::SingleDashLine)
                ...
                $frontDoorName = $_.Name
                    ...
                    $frontDoorSecurityPolicies +=  $SecurityPolicies | Select-Object @{N='FrontDoorId';E={$frontDoorId}},
                                                                                        ...
                                                                                        @{N='IsWAFPolicyInPrevention';E=
                                                                                        {
                                                                                                ...
                                                                                                if($wafPolicy.PolicyMode -ne "Prevention")
                                                                                                {
                                                                                                    $false
                                                                                                }
                                                                                        }
    ...
    $validfrontDoorSecurityPolicyDetails| ForEach-Object {
            ...
            { 
                ...
                if($frontDoorProfile -ne $null)
                {
                    ...
                    {
                        Write-Host "Error fetching Security Polcies of Front Door CDNs(s) resource. Error: $($_)" -ForegroundColor $([Constants]::MessageType.Error)
                    }
                }
                else
                {
            ...
            {
                Write-Host "Error fetching Front Door CDNs/ Security Polcies of Front Door CDNs(s) resource: Resource ID:  [$($FrontDoorName)]. Error: $($_)" -ForegroundColor $([Constants]::MessageType.Error)
            }            
        }
    }    
    ...
    # list for storing Front Door CDNs Security Policy(s) for which WAF policy mode is not Prevention.
    $SecurityPolicyWithoutWAFInPreventionMode = @()

    Write-Host "Separating Security Policy(s) for which WAF Policy mode is not in Prevention..." -ForegroundColor $([Constants]::MessageType.Info)

    $frontDoorSecurityPolicies | ForEach-Object {
        $policy = $_        
            if($_.IsWAFPolicyInPrevention -eq $false)
            {
                $SecurityPolicyWithoutWAFInPreventionMode += $policy
            }
    }
   
    $totalSecurityPolicyWithoutWAFInPreventionMode  = ($SecurityPolicyWithoutWAFInPreventionMode | Measure-Object).Count

    if ($totalSecurityPolicyWithoutWAFInPreventionMode  -eq 0)
    {
        Write-Host "No Security Policy of Front Door CDN(s) found where WAF is not in Prevention mode.. Exiting..." -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host $([Constants]::DoubleDashLine)	
        return
    }

    Write-Host "Found [$($totalSecurityPolicyWithoutWAFInPreventionMode)] Security Policy of Front Door CDN(s) found where WAF is not in Prevention mode ." -ForegroundColor $([Constants]::MessageType.Update)
    Write-Host $([Constants]::SingleDashLine)	
    ...
    $colsProperty = @{Expression={$_.FrontDoorId};Label="FrontDoorId";Width=30;Alignment="left"},
                    ...
                    @{Expression={$_.IsWAFPolicyInPrevention};Label="IsWAFPolicyInPrevention";Width=100;Alignment="left"},
    ...
    Write-Host "Frond Door CDN(s) Security Policy where WAF policy mode is not in Prevention are as follows:"
    $SecurityPolicyWithoutWAFInPreventionMode | Format-Table -Property $colsProperty -Wrap
    Write-Host $([Constants]::SingleDashLine)

    # Back up snapshots to `%LocalApplicationData%'.
    $backupFolderPath = "$([Environment]::GetFolderPath('LocalApplicationData'))\AzTS\Remediation\Subscriptions\$($context.Subscription.SubscriptionId.replace('-','_'))\$($(Get-Date).ToString('yyyyMMddhhmm'))\SetPolicyModeToPrevention"
    ...
    Write-Host $([Constants]::SingleDashLine)
     ...
     if ([String]::IsNullOrWhiteSpace($FilePath))
    {
        # Backing up Security Policy(s) details.
        $backupFile = "$($backupFolderPath)\FrontDoorCDNSecurityPolicyDetailsBackUp.csv"
        $SecurityPolicyWithoutWAFInPreventionMode | Export-CSV -Path $backupFile -NoTypeInformation
    ...
    {
        Write-Host $([Constants]::DoubleDashLine)
        Write-Host "[Step 4 of 4] Change the Policy Mode configured on Front Door CDNs(s) in the Subscription..." 
        Write-Host $([Constants]::SingleDashLine)

        if (-not $Force)
        {
            Write-Host "Do you want to change the Policy Mode configured on Front Door CDNs(s) in the Subscription? " -ForegroundColor $([Constants]::MessageType.Warning)
            
            $userInput = Read-Host -Prompt "(Y|N)"

            if($userInput -ne "Y")
            {
                Write-Host "we are not changing the Policy Mode configured on Front Door CDNs(s) in the Subscription. Exiting..." -ForegroundColor $([Constants]::MessageType.Warning)
                Write-Host $([Constants]::DoubleDashLine)	
                return
            }
        }

        # List for storing remediated Security Policy(s)
        $FrontDoorSecurityPolicyRemediated = @()

        # List for storing skipped Security Policy(s)
        $FrontDoorSecurityPolicySkipped = @()

        Write-Host "Changing the policy mode configured on Front Door CDN(s)..." -ForegroundColor $([Constants]::MessageType.Info)
        Write-Host $([Constants]::SingleDashLine)
        # Loop through the list of Security Policy(s) which needs to be remediated.
        $SecurityPolicyWithoutWAFInPreventionMode | ForEach-Object {
            $securityPolicy = $_
            try
            {
                $policy = Get-AzFrontDoorWafPolicy -ResourceGroupName $securityPolicy.WAFPolicyRgName -Name $securityPolicy.WAFPolicyName
                  if($policy -ne $null)
                    {
                        $updatedpolicy = Update-AzFrontDoorWafPolicy -Name $securityPolicy.WAFPolicyName -ResourceGroupName $securityPolicy.WAFPolicyRgName -Mode Prevention
                        if($updatedpolicy.PolicyMode -eq "Prevention")
                            {
                            $securityPolicy.IsWAFPolicyInPrevention = $true
        ...
        {
            Write-Host "Successfully changed the policy mode to Prevention configured on Front Door CDNs(s) in the subscription:" -ForegroundColor $([Constants]::MessageType.Update)
            Write-Host $([Constants]::SingleDashLine)
        ...
        if ($($FrontDoorSecurityPolicySkipped | Measure-Object).Count -gt 0)
        {
            ...
            Write-Host "Error while changing WAF policy mode Front Door CDNs(s) in the subscription:" -ForegroundColor $([Constants]::MessageType.Error)
            Write-Host $([Constants]::SingleDashLine)
...
function SetDetectionMode-OnWAFPolicyOfAzureFrontDoorCDN
{
    <#
    .SYNOPSIS
    ...
    .INPUTS
    None. You cannot pipe objects to SetDetectionMode-OnWAFPolicyOfAzureFrontDoorCDN.

    .OUTPUTS
    None. SetDetectionMode-OnWAFPolicyOfAzureFrontDoorCDN does not return anything that can be piped and used as an input to another command.

    .EXAMPLE
    PS> SetDetectionMode-OnWAFPolicyOfAzureFrontDoorCDN -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -FilePath C:\AzTS\Subscriptions\00000000-xxxx-0000-xxxx-000000000000\202109131040\ChangePolicyMode\RemediatedFrontDoorCDNPolicy.csv
    ...
    Write-Host "To change the WAF Policy mode to Detection configured on Front Door CDN(s) in a Subscription, Contributor or higher privileged role assignment on the Front Door CDNs(s) is required." -ForegroundColor $([Constants]::MessageType.Warning)
    ...
    # Back up snapshots to `%LocalApplicationData%'.
    $backupFolderPath = "$([Environment]::GetFolderPath('LocalApplicationData'))\AzTS\Remediation\Subscriptions\$($context.Subscription.SubscriptionId.replace('-','_'))\$($(Get-Date).ToString('yyyyMMddhhmm'))\SetModeToDetectionOfFrontDoorSecurtiyPolicy"
    ...
    Write-Host $([Constants]::DoubleDashLine)
    Write-Host "[Step 3 of 3] Change Policy Mode of all remediated Security Policy(s) of Front Door CDN(s)in the Subscription"
    Write-Host $([Constants]::SingleDashLine)

    if( -not $Force)
    {
        
    Write-Host "Do you want to change WAF Policy Mode of Security Policy(s) mentioned in the file?"  -ForegroundColor $([Constants]::MessageType.Warning)
    $userInput = Read-Host -Prompt "(Y|N)"

    if($userInput -ne "Y")
    {
        Write-Host "WAF policy mode will not be rolled back on Security Policy(s) of Front Door CDN(s) mentioned in the file. Exiting..." -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host $([Constants]::DoubleDashLine)
        return
    }
        Write-Host "WAF policy mode will be rolled back on Security Policy(s) of Front Door CDN(s) mentioned in the file." -ForegroundColor $([Constants]::MessageType.Update)
        Write-Host $([Constants]::SingleDashLine)
    }
    else
    {
        Write-Host "'Force' flag is provided. WAF policy mode will be rolled back on Security Policy(s) of Front Door CDN(s) in the Subscription without any further prompts." -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host $([Constants]::SingleDashLine)
    }
    ...
    $validFrontDoorSecurityPolicyDetails | ForEach-Object {
        ...
        {   
            ...
            if($policy -ne $null)
            {
                $updatedpolicy = Update-AzFrontDoorWafPolicy -Name $securityPolicy.WAFPolicyName -ResourceGroupName $securityPolicy.WAFPolicyRgName -Mode Detection
                if($updatedpolicy.PolicyMode -ne "Prevention")
                {
                    $securityPolicy.IsWAFPolicyInPrevention = $false
    ...
    {
        Write-Host "WAF Policy mode has been changed on the following Front Door CDN(s) in the Subscription.:" -ForegroundColor $([Constants]::MessageType.Update)
    ...
    {
        Write-Host "Error while changing WAF Policy mode of Front Door CDN(s) in the Subscription.:" -ForegroundColor $([Constants]::MessageType.Error)
