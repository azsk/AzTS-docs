<###
# Overview:
    This script is used to disable local authentication for Service Bus Namespaces in a Subscription.

# Control ID:
    Azure_ServiceBus_AuthN_Disable_Local_Auth

# Display Name:
    Service Bus Namespaces must not use local authentication

# Prerequisites:
    1. Contributor or higher privileges on the Service Bus Namespaces in a Subscription.
...
# Instructions to execute the script:
    To remediate:
        ...
        3. Execute the script to disable local authentication on all Service Bus Namespaces in the Subscription. Refer `Examples`, below.

    To roll back:
        1. Download the script.
        2. Load the script in a PowerShell session. Refer https://aka.ms/AzTS-docs/RemediationscriptExcSteps to know more about loading the script.
        3. Execute the script to enable local authentication on in all Service Bus Namespaces in the Subscription. Refer `Examples`, below.

# Examples:
    To remediate:
        1. To review the Service Bus Namespaces in a Subscription that will be remediated:
           Disable-AzServiceBusLocalAuth -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -DryRun

        2. To disable local authentication on of all Service Bus Namespaces in a Subscription:
           Disable-AzServiceBusLocalAuth -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck

        3. To disable local authentication on of all Service Bus Namespaces in a Subscription, from a previously taken snapshot:
           Disable-AzServiceBusLocalAuth -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -FilePath C:\AzTS\Subscriptions\00000000-xxxx-0000-xxxx-000000000000\202109131040\Disable-LocalAuthForServiceBusNamespaces\ServiceBusWithoutLocalAuthDisabled.csv
...
function Disable-LocalAuthForServiceBusNamespaces
{
    <#
        .SYNOPSIS
        Remediates 'Azure_ServiceBus_AuthN_Disable_Local_Auth' Control.

        .DESCRIPTION
        Remediates 'Azure_ServiceBus_AuthN_Disable_Local_Auth' Control.
        Disable Local Auth for Service Bus Namespace(s) in the Subscription. 
        ...
        .INPUTS
        None. You cannot pipe objects to Disable-LocalAuthForServiceBusNamespaces.

        .OUTPUTS
        None. Disable-LocalAuthForServiceBusNamespaces does not return anything that can be piped and used as an input to another command.

        .EXAMPLE
        PS> Disable-LocalAuthForServiceBusNamespaces -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -DryRun

        .EXAMPLE
        PS> Disable-LocalAuthForServiceBusNamespaces -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck

        .EXAMPLE
        PS> Disable-LocalAuthForServiceBusNamespaces -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -InputFilePath C:\AzTS\Subscriptions\00000000-xxxx-0000-xxxx-000000000000\202209131040\DisableLocalAuthForServiceBusNamespaces\ServiceBusNamespacesInputFilePath.csv
    ...
    if ($AutoRemediation) {
        ...
        Write-Host "Fetching all Service Bus Namespace(s) details failing for the [$($controlId)] control from [$($Path)]..." -ForegroundColor $([Constants]::MessageType.Info)
        Write-Host $([Constants]::SingleDashLine)
        ...
        if (($resourceDetails | Measure-Object).Count -eq 0 -or ($validServiceBusNampespaces | Measure-Object).Count -eq 0) {
            Write-Host "No Service Bus Namespace(s) found in input json file for remediation." -ForegroundColor $([Constants]::MessageType.Error)	
            Write-Host $([Constants]::DoubleDashLine)	
            return	
        }
        ...
        $validServiceBusNampespaces | ForEach-Object { 	
            try {
                ...
                Write-Host $([Constants]::SingleDashLine)
                Write-Host "Error fetching Service Bus Namespace for resource: Resource ID:  [$($resourceId)]. Error: $($_)" -ForegroundColor $([Constants]::MessageType.Error)
            }	
        }        
    }
    else {	
        if ([String]::IsNullOrWhiteSpace($InputFilePath)) {
            Write-Host "Fetching all Service Bus Namespace(s) in Subscription: [$($context.Subscription.SubscriptionId)]..." -ForegroundColor $([Constants]::MessageType.Info)
            Write-Host $([Constants]::SingleDashLine)

            # Get all Serivce Bus Namespace(s) in a Subscription
            ...
            Write-Host "Fetching all Service Bus Namespace(s) from [$($InputFilePath)]..." -ForegroundColor $([Constants]::MessageType.Info)
            Write-Host $([Constants]::SingleDashLine)
            ...
            $validServiceBusNampespaces | ForEach-Object {
                ...
                try {                
                    ...
                    Write-Host "Error fetching Service Bus Namespace details for resource: Resource ID:  [$($resourceId)]. Error: $($_)" -ForegroundColor $([Constants]::MessageType.Error)
    ...
    if ($serviceBusNamespacesCount -eq 0) {
        Write-Host "No Service Bus Namespace(s) found. Exiting..." -ForegroundColor $([Constants]::MessageType.Update)
        Write-Host $([Constants]::DoubleDashLine)	
        return
    }

    Write-Host "Found [$($serviceBusNamespacesCount)] Service Bus Namespace(s)." -ForegroundColor $([Constants]::MessageType.Update)
    ...
    $serviceBusNamespacesDetails | ForEach-Object {
        if (-not $_.DisableLocalAuth) {
            ...
            $logResource.Add("ResourceGroupName", ($_.ResourceGroupName))
            $logResource.Add("ResourceName", ($_.ResourceName))
            $logResource.Add("Reason", "Disable local authentication on the resource is already secure.")    
            $logSkippedResources += $logResource
        }
    }
    ...
    Write-Host "Found [$($actionableServiceBusNamespacesDetailsCount )] Service Bus Namespace(s) for which non secure local authentication is enabled ." -ForegroundColor $([Constants]::MessageType.Update)
    Write-Host $([Constants]::SingleDashLine)	
    ...
    $colsProperty = @{Expression = { $_.ResourceName }; Label = "ResourceName"; Width = 30; Alignment = "left" },
                    ...
                    @{Expression = { $_.DisableLocalAuth }; Label = "DisableLocalAuth"; Width = 100; Alignment = "left" }
    ...
    $backupFolderPath = "$([Environment]::GetFolderPath('LocalApplicationData'))\AzTS\Remediation\Subscriptions\$($context.Subscription.SubscriptionId.replace('-','_'))\$($(Get-Date).ToString('yyyyMMddhhmm'))\Disable-LocalAuthForServiceBusNamespaces"

    if (-not (Test-Path -Path $backupFolderPath)) {
        New-Item -ItemType Directory -Path $backupFolderPath | Out-Null
    }

    Write-Host "[Step 3 of 4] Back up Service Bus Namespace(s) details"
    Write-Host $([Constants]::SingleDashLine)
    ...
    if (-not $DryRun) {
        ...
        Write-Host $([Constants]::DoubleDashLine)
        Write-Host "[Step 4 of 4] To disable Local authentication on Service Bus Namespace(s) in the Subscription" 
        Write-Host $([Constants]::SingleDashLine)

        if (-not $AutoRemediation) {
            if (-not $Force) {
                Write-Host "Do you want to disable local authentication on Service Bus Namespace(s) in the Subscription? " -ForegroundColor $([Constants]::MessageType.Warning)

                $userInput = Read-Host -Prompt "(Y|N)"

                if ($userInput -ne "Y") {
                    Write-Host "Local authentication will not be changed for Service Bus Namespace(s) in the Subscription. Exiting..." -ForegroundColor $([Constants]::MessageType.Warning)
                    Write-Host $([Constants]::DoubleDashLine)	
                    return
                }
            }
         ...
         # List for storing remediated Service Bus Namespace(s)
        $serviceBusNamespacesRemediated = @()

        # List for storing skipped Service Bus Namespace(s)
        $serviceBusNamespacesSkipped = @()

        Write-Host "Setting secure local authentication on Service Bus Namespace(s)..." -ForegroundColor $([Constants]::MessageType.Info)
        Write-Host $([Constants]::SingleDashLine)
        ...
        $serviceBusWithLocalAuthEnabled  | ForEach-Object {
            try {
                ...
                if ($disabledLocalAuth) {
                    ...
                    $logResource.Add("ResourceGroupName", ($_.ResourceGroupName))	
                    $logResource.Add("ResourceName", ($_.ResourceName))
                    $logResource.Add("Reason", "Error while setting DisableLocalAuth: [$($_)]")            
                    $logSkippedResources += $logResource	
                ...
                $logResource.Add("ResourceGroupName", ($_.ResourceGroupName))	
                $logResource.Add("ResourceName", ($_.ResourceName))	
                $logResource.Add("Reason", "Encountered error while setting DisableLocalAuth")    	
                $logSkippedResources += $logResource
            }
        }
        ...
        if ($AutoRemediation) {
            if ($($serviceBusNamespacesRemediated | Measure-Object).Count -gt 0) {
                ...
                Write-Host "The information related to Service Bus Namespace(s) where disable local authentication was set, has been saved to [$($serviceBusNamespacesRemediatedFile)]. Use this file for any roll back that may be required." -ForegroundColor $([Constants]::MessageType.Update)
                Write-Host $([Constants]::SingleDashLine)
            }
            ...
            if ($($serviceBusNamespacesSkipped | Measure-Object).Count -gt 0) {
                ...
                Write-Host "The information related to Service Bus Namespace(s) where disable local authentication was not set, has been saved to [$($serviceBusNamespacesSkippedFile)]. Use this file for any roll back that may be required." -ForegroundColor $([Constants]::MessageType.Warning)
                Write-Host $([Constants]::SingleDashLine)
            }
            ...
            if ($($serviceBusNamespacesSkipped | Measure-Object).Count -gt 0) {
                ...
                Write-Host "Error while setting up the disable local authentication for the following Service Bus Namespaces(s) in the subscription:" -ForegroundColor $([Constants]::MessageType.Error)
                Write-Host $([Constants]::SingleDashLine)
        ...
        Write-Host "[Step 4 of 4] disable local authentication on Service Bus Namespaces(s) in the Subscription" 
        Write-Host $([Constants]::SingleDashLine)
        Write-Host "Skipped as -DryRun switch is provided." -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host $([Constants]::SingleDashLine)
        Write-Host "Next steps:" -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host "Run the same command with -InputFilePath $($backupFile) and without -DryRun, to disable local authentication for Service Bus Namespaces(s) listed in the file." -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host $([Constants]::DoubleDashLine)
    }   
}

function Reset-LocalAuthForServiceBusNamespaces {
     <#
        .SYNOPSIS
        Rolls back remediation done for 'Azure_ServiceBus_AuthN_Disable_Local_Auth' Control.

        .DESCRIPTION
        Rolls back remediation done for 'Azure_ServiceBus_AuthN_Disable_Local_Auth' Control.
        Change disable local authentication to Previous Value on Service Bus Namespace(s) in the Subscription. 
        ...
        .INPUTS
        None. You cannot pipe objects to function Reset-LocalAuthForServiceBusNamespaces.

        .OUTPUTS
        None. function Reset-LocalAuthForServiceBusNamespaces does not return anything that can be piped and used as an input to another command.
        
        .EXAMPLE
        PS> Reset-LocalAuthForServiceBusNamespaces -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck -RollbackFilePath C:\AzTS\Subscriptions\00000000-xxxx-0000-xxxx-000000000000\202109131040\DisableLocalAuthForServiceBusNamespaces\RemediatedServiceBusNamespaces.csv
    ...
    Write-Host "Fetching all Service Bus Namespace(s) from" -NoNewline
    ...
    if ($validServiceBusNamespacesDetailsCount -eq 0) {
        Write-Host "No Service Bus Namespace(s) found. Exiting..." -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host $([Constants]::DoubleDashLine)	
        return
    }

    Write-Host "Found [$($validServiceBusNamespacesDetailsCount)] Service Bus Namespace(s)." -ForegroundColor $([Constants]::MessageType.Update)
    Write-Host $([Constants]::SingleDashLine)
    ...
    # Back up snapshots to `%LocalApplicationData%'.
    $backupFolderPath = "$([Environment]::GetFolderPath('LocalApplicationData'))\AzTS\Remediation\Subscriptions\$($context.Subscription.SubscriptionId.replace('-','_'))\$($(Get-Date).ToString('yyyyMMddhhmm'))\Reset-DisableLocalAuthForServiceBusNamespaces"
    ...
    if (-not $Force) {
        ...
        if ($userInput -ne "Y") {
            Write-Host "Local authentication will not be rolled back on Service Bus Namespace(s) mentioned in the file. Exiting..." -ForegroundColor $([Constants]::MessageType.Warning)
            Write-Host $([Constants]::DoubleDashLine)
            return
        }
        Write-Host "Local authentication will be rolled back on Service Bus Namespace(s) mentioned in the file." -ForegroundColor $([Constants]::MessageType.Update)
        Write-Host $([Constants]::SingleDashLine)
    }
    else {
        Write-Host "'Force' flag is provided. Local authentication will be rolled back on Service Bus Namespace(s) in the Subscription without any further prompts." -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host $([Constants]::SingleDashLine)
    }
    ...
    $validServiceBusNamespacesDetails | ForEach-Object {
        try {   
            Set-AzServiceBusNamespace -ResourceGroupName $_.ResourceGroupName -Name $_.ResourceName -DisableLocalAuth:$disableLocalAuthValue
    ...
    if ($($serviceBusNamespacesRolledBack | Measure-Object).Count -gt 0) {
        Write-Host "Local authentication has been rolled back on the following Service Bus Namespace(s) in the Subscription.:" -ForegroundColor $([Constants]::MessageType.Update)
    ...
    if ($($serviceBusNamespacesSkipped | Measure-Object).Count -gt 0) {
        Write-Host "Error while rolling back local authentication on Service Bus Namespace(s) in the Subscription.:" -ForegroundColor $([Constants]::MessageType.Error)
        $serviceBusNamespacesSkipped | Format-Table -Property $colsProperty -Wrap
        Write-Host $([Constants]::SingleDashLine)
