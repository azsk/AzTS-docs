...
function Enable-AADForKubernetes
{
    ...
    Write-Host "NOTE: To enable AAD for Kubernetes Services in a Subscription, Contributor or higher privileges on the Kubernetes Services are required." -ForegroundColor $([Constants]::MessageType.Warning)
    Write-Host $([Constants]::SingleDashLine)
    ...
    if (-not $DryRun)
    {
        ...
        { 
            ...
            if ($($kubernetesClusterRemediated | Measure-Object).Count -gt 0)
            {
                Write-Host "AAD is successfully enabled on the following Kubernetes cluster(s) in the subscription:"
                Write-Host $([Constants]::SingleDashLine)
            ...
            if ($($kubernetesClusterSkipped | Measure-Object).Count -gt 0)
            {
                Write-Host "Error enabling AAD on the following Kubernetes cluster(s) in the subscription:"
                Write-Host $([Constants]::SingleDashLine)
