<###
# Overview:
    This script is used to enable Entra Id for Kubernetes Services in a Subscription.

# Control ID:
    Azure_KubernetesService_AuthN_Enabled_Microsoft_Entra_Id

# Display Name:
    Entra Id should be enabled in Kubernetes Service.
...
# Steps performed by the script:
    To remediate:
        ...
        2. Get the list of Kubernetes Services in a Subscription that do not have Entra Id enabled.
        3. Back up details of Kubernetes Services that are to be remediated.
        4. Enable Entra Id in all Kubernetes Services in the Subscription.

# Instructions to execute the script:
    To remediate:
        1. Download the script.
        2. Load the script in a PowerShell session. Refer https://aka.ms/AzTS-docs/RemediationscriptExcSteps to know more about loading the script.
        3. Execute the script to enable Entra Id in all Kubernetes Services in the Subscription. Refer `Examples`, below.
...
# Examples:
    To remediate:
        ...
        2. To enable Entra Id in all Kubernetes Services in a Subscription:
           Enable-AADForKubernetes -SubscriptionId 00000000-xxxx-0000-xxxx-000000000000 -PerformPreReqCheck

        3. To enable Entra Id in all Kubernetes Services in a Subscription, from a previously taken snapshot:
        ...
        **Note: If you want to add Entra Id group to AKS cluster, please provide ADD group object id for each of the cluster.
...
function Enable-AADForKubernetes
{
    ...
    Write-Host "To enable Entra Id for Kubernetes Services in a Subscription, Contributor or higher privileges on the Kubernetes Services are required." -ForegroundColor $([Constants]::MessageType.Warning)
    Write-Host $([Constants]::SingleDashLine)
    ...
    Write-Host $([Constants]::SingleDashLine)
    # Includes Kubernetes Services where Entra Id is disabled.
    ...
    $kubernetesServiceResources | ForEach-Object {
        ...
        try
        {
            ...
            {    
                ...
                Write-Host "Skipping this Kubernetes Service with resource name: [$($_.Name)] and resource group name: [$($_.ResourceGroupName)] as Entra Id is already enabled..." -ForegroundColor $([Constants]::MessageType.Update)
                Write-Host $([Constants]::SingleDashLine)
                $logResource = @{}
                $logResource.Add("ResourceGroupName",($_.ResourceGroupName))
                $logResource.Add("ResourceName",($_.Name))
                $logResource.Add("Reason","Entra Id is already enabled.")    
                $logSkippedResources += $logResource
            }
        }
        catch
    ...
    {
        Write-Host "No Kubernetes Service found with Entra Id disabled. Exiting..." -ForegroundColor $([Constants]::MessageType.Update)
        Write-Host $([Constants]::DoubleDashLine)
    ...
    Write-Host "Found [$($totalKubernetesServicesWithoutAADEnabled)] out of [$($totalKubernetesServices)] Kubernetes Service(s) with Entra Id disabled." -ForegroundColor $([Constants]::MessageType.Update)
    Write-Host $([Constants]::SingleDashLine)
    ...
    if (-not $DryRun)
    {
        Write-Host "[Step 5 of 5] Enable Entra Id for Kubernetes Services"
        Write-Host $([Constants]::SingleDashLine)
        ...
        $colsProperty = @{Expression={$_.Id};Label="Resource ID";Width=40;Alignment="left"},
                        ...
                        @{Expression={$_.IsAADEnabled};Label="Is Entra Id enabled on Kubernetes Cluster?";Width=20;Alignment="left"}

        Write-Host "Following Kubernetes cluster(s) are having Entra Id disabled:" #-ForegroundColor $([Constants]::MessageType.Warning)
        ...
        Write-Host "1. RBAC must be enabled on Kubernetes cluster to enable Entra Id." -ForegroundColor $([Constants]::MessageType.Warning) 
        Write-Host "2. Once AAD is enabled, you won't be able to disable it again." -ForegroundColor $([Constants]::MessageType.Warning) 
        Write-Host "`nFor more information on using Entra Id authentication in AKS, please refer: https://docs.microsoft.com/en-us/azure/aks/azure-ad-rbac" -ForegroundColor $([Constants]::MessageType.Info)
        Write-Host $([Constants]::SingleDashLine)
        Write-Host "Do you want to enable Entra Id for all Kubernetes Services? " -NoNewline

        $userInput = Read-Host -Prompt "(Y|N)"
        Write-Host $([Constants]::SingleDashLine)
        if($userInput -ne "Y")
        {
            Write-Host "Entra Id will not be enabled for any Kubernetes Services. Exiting..." -ForegroundColor $([Constants]::MessageType.Update)
            Write-Host $([Constants]::DoubleDashLine)
            return
        }
        Write-Host "User has provided consent to enable Entra Id for all Kubernetes Services" -ForegroundColor $([Constants]::MessageType.Update)
        Write-Host $([Constants]::SingleDashLine)
        ...
        $kubernetesServicesWithoutAADEnabled | ForEach-Object {
            ...
            Write-Host "Enabling Entra Id for Kubernetes cluster [$($resourceName)]..." -ForegroundColor $([Constants]::MessageType.Info)
            Write-Host $([Constants]::SingleDashLine)
            # Check whether RBAC is enabled on cluster or not. Without RBAC, Entra Id cannot be enabled.
            if ($isRBACEnabled -eq $true)
            {
                ...
                {
                    Write-Host "Entra Id group will not be added to Kubernetes cluster [$($resourceName)]." -ForegroundColor $([Constants]::MessageType.Update)
                    Write-Host $([Constants]::SingleDashLine)
                }

                # Setting Entra Id profile config to Kubernetes cluster.
                ...
                if ($res.properties.aadProfile.managed)
                {
                    ...
                    Write-Host "Successfully enabled Entra Id for Kubernetes cluster [$($resourceName)]" -ForegroundColor $([Constants]::MessageType.Update)
                    Write-Host $([Constants]::SingleDashLine)
                }
                else
                {
                    ...
                    $logResource.Add("ResourceGroupName",($_.ResourceGroupName))
                    $logResource.Add("ResourceName",($_.Name))
                    $logResource.Add("Reason","Error occurred enabling Entra Id for Kubernetes cluster [$($resourceName)].")    
                    $logSkippedResources += $logResource
                    Write-Host "Error occurred enabling Entra Id for Kubernetes cluster [$($resourceName)]" -ForegroundColor $([Constants]::MessageType.Error)
                    Write-Host $([Constants]::SingleDashLine)
                }
            }
            else
            {
                ...
                $logResource.Add("ResourceGroupName",($_.ResourceGroupName))
                $logResource.Add("ResourceName",($_.Name))
                $logResource.Add("Reason","Entra Id can't be enabled for this resource since RBAC is disabled.")    
                $logSkippedResources += $logResource
                Write-Host "Entra Id can't be enabled for this resource since RBAC is disabled." -ForegroundColor $([Constants]::MessageType.Error)
                Write-Host $([Constants]::SingleDashLine)
                return
            }
        }
        ...
        if($AutoRemediation)
        {
            if ($($kubernetesClusterRemediated | Measure-Object).Count -gt 0)
            {
                ...
                Write-Host "Entra Id is enabled on the Kubernetes cluster(s)." -ForegroundColor $([Constants]::MessageType.Update)
            ...
            if ($($kubernetesClusterSkipped | Measure-Object).Count -gt 0)
            {
                ...
                Write-Host "Error enabling Entra Id on some Kubernetes cluster(s)." -ForegroundColor $([Constants]::MessageType.Error)
        ...
        { 
            ...
            if ($($kubernetesClusterRemediated | Measure-Object).Count -gt 0)
            {
                Write-Host "Entra Id is successfully enabled on the following Kubernetes cluster(s) in the subscription:" -ForegroundColor $([Constants]::MessageType.Update)
            ...
            if ($($kubernetesClusterSkipped | Measure-Object).Count -gt 0)
            {
                Write-Host "`nError enabling Entra Id on the following Kubernetes cluster(s) in the subscription:" -ForegroundColor $([Constants]::MessageType.Error)
    ...
    else
    {
        ...
        Write-Host "Run the same command with -FilePath $($backupFile) and without -DryRun, to enable Entra Id for all Kubernetes Service resources listed in the file." -ForegroundColor $([Constants]::MessageType.Warning)
        Write-Host $([Constants]::SingleDashLine)
    }
}
...
class SetAADProfile
{
    ...
    [PSObject] SetAADProfileForCluster([string] $subscriptionId, [string] $resourceName, [string] $resourceGroup, [string] $location, [Object[]] $aadClientIds)
    {
        ...
        try
        {
            ...
            # If user wants to add Entra Id Group to Kubernetes cluster, pass group object id as part of request body.
            if (![String]::IsNullOrWhiteSpace($aadClientIds))
            {
                $body =@'
	            {
...
'@
            ...
            # API to set Entra Id Profile config to Kubernetes cluster
            $response = Invoke-WebRequest -Method $method -Uri $armUri -Headers $headers -Body $jsonString -UseBasicParsing
            $content = ConvertFrom-Json $response.Content
        }
        catch
        {
            Write-Host "Error occurred while setting Entra Id profile to Kubernetes Cluster. ErrorMessage [$($_)]" -ForegroundColor $([Constants]::MessageType.Error)
        }
