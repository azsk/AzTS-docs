...
>
>     **Note:** Enabling VNet integration for AzTS setup is not recommended as of now due to ongoing issue for connectivity to Application Insights over Private link scope
>
...
|EnableVnetIntegration | Switch to enable VNet integration for AzTS setup. Enabling VNet integration for AzTS setup, ensures that all critical resources like storage, function apps, log analytics workspace etc that are part of AzTS setup, are not accessible over public internet. </br>**Note:** Enabling VNet integration for AzTS setup is not recommended as of now due to ongoing issue for connectivity to Application Insights over Private link scope.|FALSE|
